...
cdef extern from "pTau.hpp":
    cdef void integrate_alpha_double "integrate_alpha<double>"(long nPix, long nDep, const double* const z,
		     const double* const  alpha, double* const ltau, int nthreads)
    cdef void integrate_alpha_float "integrate_alpha<float>"(long nPix, long nDep, const float* const z,
    ...
    cdef void getAlpha_T_Ne_double "getAlpha_T_Ne<double>"(long ntot, const double* const Tg, const double* const Ne, const double* const nH,
		    int  nLambda, const double* const  lambd, double* const alpha, int  nthreads)
    cdef void getAlpha_T_Ne_float "getAlpha_T_Ne<float>"(long ntot, const float* const Tg, const float* const Ne, const float* const nH,
		    int  nLambda, const double* const  lambd, double* const alpha, int  nthreads)
...
cdef extern from "gradients.hpp":
    cdef void optimize_gradients_float "gr::optimizeGradients<float>"(long nPix, long nDep, const float* const temp, const float* const ltau,
                                                                      const float* const rho, const float* const vlos, int smooth_window,
                                                                      float Tcut, float tau_cut, int nthreads, long nDep2, float* const res, float vel_scal,
                                                                      float ltau_top)

    cdef void optimize_gradients_double "gr::optimizeGradients<double>"(long nPix, long nDep, const double* const temp, const double* const ltau,
                                                                        const double* const rho, const double* const vlos, int smooth_window,
                                                                        double Tcut, double tau_cut, int nthreads, long nDep2, double* const res, double vel_scal,
                                                                        double ltau_top)
    
    cdef void interpolateGradient_double "gr::interpolateGradient<double>"(long nPix, long nDep, const double* const  var, long  nDep2,
                                                                           const double* const  index, const double* const index_new, double* const res,
                                                                           int  nthreads)
    
    cdef void interpolateGradient_float "gr::interpolateGradient<float>"(long  nPix, long  nDep, const float* const  var, long  nDep2,
...
def getBackgroundOpacityPgas_float(ar[float,ndim=3] Tg, ar[float,ndim=3] Pg, ar[double,ndim=1] wav, int nthreads = 4):
    ...
    cdef long ny = Tg.shape[0]
    cdef long nx = Tg.shape[1]
    cdef long nDep = Tg.shape[2]
...
# ***********************************************************************************************
def getBackgroundOpacityNe_float(ar[float,ndim=3] Tg, ar[float,ndim=3] Ne, ar[float, ndim=4] nH, ar[double,ndim=1] wav, int nthreads = 4):
    ...
    # call C++ tools
    getAlpha_T_Ne_float(ntot, <float*>Tg.data, <float*>Ne.data, <float*>nH.data, nwav, <double*>wav1.data, <double*>alpha.data, <int>nthreads)
...
def getBackgroundOpacityNe_double(ar[double,ndim=3] Tg, ar[double,ndim=3] Ne, ar[double,ndim=4] nH, ar[double,ndim=1] wav, int nthreads = 4):
    ...
    # call C++ tools
    getAlpha_T_Ne_double(ntot, <double*>Tg.data, <double*>Ne.data, <double*>nH.data, nwav, <double*>wav1.data, <double*>alpha.data, <int>nthreads)
...
def getTau_float(ar[float,ndim=3] z, ar[double, ndim=3] alpha, int nthreads=4):
    ...
    # Dimensions
    cdef long ny = z.shape[0]
    cdef long nx = z.shape[1]
    cdef long nDep = z.shape[2]
...
def OptimizeGradients_double(ar[double,ndim=3] temp, ar[double,ndim=3] rho, ar[double,ndim=3] vlos, ar[double, ndim=3] ltau, int nthreads=4, nDep2 = None, int smooth_window = 1, double Tcut = 50000.0, double ltau_cut = 2.0, double vel_scal=4.0, double ltau_top = -15.0):
    ...
    cdef long ny = temp.shape[0]
    cdef long nx = temp.shape[1]
    cdef long nDep = temp.shape[2]
    cdef long nDep_new = nDep
...
def interpolate_gradient_double(ar[double, ndim=3] index_new, ar[double, ndim=3] var, int nthreads=4):
    ...
    cdef long ny = var.shape[0]
    cdef long nx = var.shape[1]
    cdef long nDep = var.shape[2]
    cdef long nDep_new = index_new.shape[2]
    cdef long nPix = ny*nx

...
namespace phyc{
  ...
  template<typename T = double> static constexpr T CC   = 2.99792458E+10;            // Speed of light [cm/s]
  template<typename T = double> static constexpr T BK   = 1.3806488E-16;             // Boltzmann constant [erg/K]
  template<typename T = double> static constexpr T QE   = 4.80320441E-10;           // electron charge 
  template<typename T = double> static constexpr T ME   = 9.10938188E-28;            // electron mass [g]
  template<typename T = double> static constexpr T SQRTME   = 3.018175256674138e-14;        // SQRT electron mass [g]
  template<typename T = double> static constexpr T  EV  = 1.60217733E-12;            // One electronVolt 
 
  template<typename T = double> static constexpr T AMU  = 1.660538921E-24;           // Atomic mass unit [g]
  template<typename T = double> static constexpr T HH   = 6.62606957E-27;            // Planck constant [erg s]
  template<typename T = double> static constexpr T EPS0 = 1/(4*PI<T>);           // Vacuum permittivity 
  template<typename T = double> static constexpr T MU0  = 1;//1.2566370614E-06;          // Magnetic induct. of vac.
  //template<typename T = double> static constexpr T RBOHR= 5.29177349E-11;            // Bohr radius [cm]           

  //template<typename T = double> static constexpr T Ryd  = 10973731.568508;           // Rydberg constant [cm^-1]
